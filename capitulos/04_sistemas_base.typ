...
#figure(
  ...
  caption: [Evolución temporal de la probabilidad de excitación $Pe (t)$ para un átomo de dos niveles interactuando con un campo clásico, partiendo del estado base $kg$. Las líneas continuas corresponden a la solución analítica de la @eq:1at2lvl_analitica, mientras que los marcadores representan los resultados obtenidos numéricamente usando la integración de la ecuación maestra (@eq:1at2lvl_maestra) con OpenKet. Se observan tres valores de desintonía: total resonancia $Dac = 0.0$ (círculos), y fuera de resonancia $Dac = 2.0$ (triángulos), $Dac = 4.0$ (cuadrados). El tiempo está normalizado respecto a la frecuencia de Rabi $rabic$, cuyo valor es $rabic = 2.0$.],
) <fig:1at2lvl>
...
donde $rabic$ es la frecuencia de Rabi que caracteriza la intensidad de acomplamiento entre los niveles $kg <-> ke$, y $rabir = sqrt(rabic^2 + Dac^2)$ es la frecuencia de Rabi generalizada, la cual depende de la desintonía $Dac$ entre la frecuencia del láser y la de transición atómica.
...
Ahora, el siguiente paso es simular la inyección y pérdida de excitaciones en una cavidad abierta, implementando una evolución no unitaria. Para ello, se consideró a la cavidad en ausencia de átomos.
...
Para caracterizar la respuesta de la cavidad, el observable de interés es el número medio de fotones dentro en el estado estacionario, $Nss$. Sin embargo, en la práctica esta cantidad no es físicamente accesible, pero la teoría de _input-output_ @collett_squeezing_1984 expresa que el flujo de fotones que salen de la cavidad a través de un espejo transmisivo, es directamente proporcional a la tasa de disipasión $kappa$ y el número de fotones almacenados en ese intante. Por lo tanto, el observable $Nss$ nos brinda una medida directa y proporcional de su espectro de transmisión.
...
El perfil de la curva obtenida corresponde a una Lorentziana centrada en $Dpa=0$, característica de una cavidad óptica @carmichael_statistical_1999[$section 1.5.3$]. Es decir, la máxima inyección de fotones ocurre cuando el láser de bombeo y el modo de la cavidad están perfectamente sincronizados. Además, conforme aumenta la magntud de desintonía, $abs(Dpa) > 0$, la probabilidad de que un fotón ingrese a la cavidad disminuye de manera significativa.

#figure(
  image("../assets/figuras/cavidad_fotones.png"),
  caption: [Espectro de trasmisión de la cavidad vacía en el estado estacionario. Se grafica el número de fotones $Nss$ en función de la desintonía del campo de bombeo $Dpa$. La curva describe una distrución Lorentziana, con un máximo de transmisión en resonancia perfecta $Dpa=0$. La desintonía está normalizada respecto a la disipación $kappa$. Los valores de los parámetros usados son $rabip = 1.0$, $kappa = 1.0$, $nmax = 10$.]
) <fig:cavidad_fotones>
