...
Hasta ahora, se ha explicado toda la dinámica utilizando vectores de estado $ket(psi)$. Lo cual no tiene nada matemáticamente incorrecto, pero supone que el sistema se encuntra aislado de su entorno y que es un estado puro.

Sin embargo, en los procesos reales, la interacción con el entorno provoca que el sistema adquiera grados de libertad que no son observables, por lo que se necesita trabajar con un enfoque estadístico #footnote[Desarrollo obtenido de @cohentannoudji_quantum_20051[$section "E.III"$] y @breuer_theory_2002[$section 3.1, section 3.2$]].
...
Cuando representamos al operador densidad en una base ortonormal discreta {$ket(u_n)$}, a los elementos diagonales de la matriz densidad, $rr_(n n)=braket(u_n,rr,u_n)$ se le denominan poblaciones y se expresan como:

$ rr_(n n) = sum_k p_k abs(braket(u_n, psi_k))^2. $

Estas poblaciones son números reales positivos tales que $sum_k rr_(n n)=1$, y físicamente representan la probabilidad de encontrar al sistema en el estado $ket(u_n)$ al realizar una medición.
