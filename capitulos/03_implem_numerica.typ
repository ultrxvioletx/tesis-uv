...
#let rr = $hat(rho)$ //matriz densidad
#let sig(ii,jj,at) = $sigma_(ii jj)^((at))$ //sigma

Para el sistema propuesto en este trabajo, el espacio de Hilbert total se construye como el producto tensorial de los subespacios individuales: $HH = HH_"cavidad" ** HH_"átomo1" ** HH_"átomo2"$. Dado que cada átomo posee 4 niveles de energía, y el espacio de Fock de la cavidad debe ser truncado un número máximo de fotones, $N_(max)$, la dimensión total del espacio de Hilbert es $D = 4^2 N_(max)$. Por lo tanto, la matriz densidad $rr(t)$ contiene $D = (16 N_(max))^2$ elementos #footnote[En realidad, la dimensión final es $2D$, debido que cada elemento de la matriz densidad tiene una parte real y una parte imaginaria, que se operan como elementos independientes.].

Para un truncamiento típico de $N_(max) = 10$, el sistema requiere la resolución simultánea de $(160)^2 = 25,600$ ecuaciones diferenciales ordinarias (EDOs) acopladas y con coeficientes complejos. Además, a medida que se incrementa el truncamiento $N_(max)$ para obtener resultados más precisos, la dimensión del espacio de Hilbert crece exponencialmente, lo que hace que la solución analítica sea inaccesible.
...
La matriz densidad $rr(t)$ contiene toda la información estadística del sistema; sin embargo, no tiene mucho significado físico directo hasta que se calculan los valores esperados de los observables.
